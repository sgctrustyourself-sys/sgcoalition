...
const BlogManager = () => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file.type.startsWith('image/')) {
            addToast('Please choose an image file before opening the cropper.', 'error');
    ...
    const handleCroppedImageUpload = async (croppedFile: File) => {
        try {
            addToast('Uploading cropped cover image...', 'info');
            const url = await uploadProductImage(croppedFile, 'blog-cover');
    ...
    return (
        <div className="bg-black/40 border border-white/5 rounded-3xl p-8 backdrop-blur-xl">
                            ...
                            <div>
                                ...
                                <div className="relative group aspect-[16/9] bg-white/5 border border-white/10 rounded-xl overflow-hidden mb-4">
                                    ...
                                    <label className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity">
                                        <span className="text-[10px] font-bold uppercase text-white">Crop & Upload</span>
            ...
            <ImageCropperModal
                ...
                description="Drag to line up the cover art before we upload it to the post."
                confirmLabel="Upload Crop"
                cancelLabel="Cancel"
