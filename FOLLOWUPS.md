...
2. **Deferred migrations** (Operator) — production was provisioned selectively, so 9 authored migrations sit in `supabase/migrations/.deferred` (the runner skips them): the retired referendum post (also incompatible with the posts table), SGCoin payout requests, wallet_mints_7d + realtime, production_state + realtime, orders paid_amount/balance_due, and the payments + reconcile RPC. Remove a line from `.deferred` to launch that migration. Three others — brain seed, brain admin-lock, subscribe_emails — were applied Aug 12 (the newsletter drop-subscribe feature was live but its table was missing; the brain lock closes public reads on the admin-only Brain). Run `npx tsx scripts/applyMigrations.ts --check` for the current state.
