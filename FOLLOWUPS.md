...
3. DONE — see the resolution and the 7/8 run below. A preview now exercises the pricing agreement.
...
**Blocker that run surfaced (pre-existing, not caused by the bypass):** the preview's `/api/payment-settings` answered **503 `Supabase admin service is not configured.`**, so the run reported `no API reachable` and failed the pricing-agreement check by design (a deployment target with no API is a failure, not a skip). Cause: every server-side Supabase variable was scoped to the **Production** environment only — `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, `SUPABASE_JWT_SECRET`, plus `ORDER_NOTIFICATION_EMAIL`, `RESEND_FROM_EMAIL` and `VITE_APP_URL`. `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` do cover preview, which is why the storefront read products and the browser flow passed while `/api/*` could not reach Supabase at all. `STRIPE_SECRET_KEY` already carried a separate preview-scoped value, so previews were half-provisioned by design rather than accidentally.

**Resolution (2026-09-16) — read-only Preview provisioning.** Preview got its own `SUPABASE_URL`, `SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY`, the last one deliberately set to the **anon key** so RLS applies and a preview holds no production-write credential. That is sufficient and verified sufficient: `pricing-preview` only reads products and coupons through `resolvePricing`, and the `payment_settings` migration grants `SELECT ... TO anon, authenticated` with a public-read policy. `SUPABASE_JWT_SECRET` was deliberately **not** given to preview — nothing in the smoke path needs it, and it is the key that forges Supabase user tokens. Production's own values are untouched (its `SUPABASE_SERVICE_ROLE_KEY` is still the real sensitive one); production `/api/health` and `/api/payment-settings` both still answer 200 after the change.

A second defect had to be fixed before the money path could run at all, and it was invisible until the API became reachable: the harness sent `x-vercel-set-bypass-cookie: true` on its **API probe** as well as in the browser context. Asking Vercel to set that cookie answers **307**, and following that redirect from Node fails outright — which the probe's `.catch()` swallowed as `no API reachable`. That is the worst possible failure shape, since it reports an auth quirk as an outage. The probe now sends the bypass header alone (`bypassProbeHeaders`); the browser context still sends both, because it genuinely needs the `_vercel_jwt` cookie for later navigations and XHRs.

**Verified after both changes** against `coalition-brand-gddzbumyh-derron-byrds-projects.vercel.app`:

- `curl` with the bypass header: `/api/payment-settings` → **200** JSON, `/api/pricing-preview` → **200** with real pricing (40000 cents for the $400 polo).
- `npm run smoke:checkout` → **SMOKE OK (partial), 7/8 passed, 1 skipped** — the skip is the plain stock-out case, which this catalogue genuinely does not list. Critically, `ok checkout total matches the server — 120 both sides`: the pricing agreement now runs **and passes on a real protected preview**, which is what this whole item existed for.

One residual: preview deployments also lack `ORDER_NOTIFICATION_EMAIL` / `RESEND_FROM_EMAIL`, so an order created on a preview would send no confirmation email. That does not affect the smoke test (it stops before payment) and is left as-is on purpose — a preview should not be able to email real customers.
