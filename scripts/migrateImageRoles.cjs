// =============================================================================
// /scripts/migrateImageRoles.cjs
// -----------------------------------------------------------------------------
// Idempotently adds the `image_roles` JSONB column to public.products. Same
// pattern as scripts/apply_tier_pricing_to_shirts.cjs: pre-flight check via
...
//
// Companion migration file: supabase/migrations/20260630_add_image_roles_to_products.sql
//
// USAGE
//   node scripts/migrateImageRoles.cjs                # pre-flight check + dry-run
//   node scripts/migrateImageRoles.cjs --apply        # actually ALTER TABLE
//
// CONNECTION MODEL
//   Mirrors scripts/apply_tier_pricing_to_shirts.cjs: pg + multi-attempt
//   pool/direct connection + dotenv from .env (gitignored).
// =============================================================================
...
async function main() {
    ...
    try {
        ...
        if (hasColumn) {
            console.log(`\n[OK] image_roles already exists on public.products (${pre.rows[0].data_type}).`);
            if (isDryRun) {
                console.log('Nothing to do. Re-run with --apply to force a no-op ALTER (idempotent).');
            }
            return;
        }

        if (isDryRun) {
            console.log('\n[DRY-RUN] image_roles column missing on public.products.');
            console.log('Pass --apply to ALTER TABLE public.products ADD COLUMN image_roles JSONB.\n');
            printSqlFallback();
            return;
        }

        console.log('\n[APPLY] Adding image_roles column to public.products.');
        await client.query(SQL_APPLY);
        console.log('Added image_roles column.');
        console.log('Re-run to confirm idempotency: node scripts/migrateImageRoles.cjs');
        console.log('Next: rebuild the storefront; getProductRoles will start returning role-aware mapping once the column is filled.');
    } catch (e) {
        if (/column .*image_roles.* already exists/i.test(e.message)) {
            console.log('[OK] image_roles already exists (caught on the ALTER).');
            return;
        }
