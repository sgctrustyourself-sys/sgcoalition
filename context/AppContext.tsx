...
import { INITIAL_SECTIONS, COIN_REWARD_RATE, INITIAL_PRODUCTS, ADMIN_WALLETS, INITIAL_ORDERS } from '../constants';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const handleSwitchToPolygon = async () => {
        try {
            const { switchToPolygon: switchFn } = await import('../services/web3Service');
    ...
    const syncCryptoBalances = async (walletAddress: string) => {
        try {
            ...
            const { getSGCoinV2Balance, getBurnedSGCoinV1 } = await import('../services/web3Service');

            const [v1, v2, bV1] = await Promise.all([
                getSGCoinBalance(walletAddress, provider).catch(() => '0'),
                getSGCoinV2Balance(walletAddress, provider).catch(() => '0'),
                getBurnedSGCoinV1(provider).catch(() => '0')
            ]);

            const v1Bal = parseFloat(v1) || 0;
            const v2Bal = parseFloat(v2) || 0;
            const totalMig = parseFloat(bV1?.replace(/,/g, '') || '0');

            setUser(prev => prev ? {
                ...prev,
                sgCoinBalance: prev.sgCoinBalance || v1Bal,
                v2Balance: v2Bal,
                totalMigrated: totalMig
            } : null);
    ...
    useEffect(() => {
        ...
        const initApp = async () => {
            try {
                        ...
                        if (metamaskAddress) {
                            ...
                            if (mounted) {
                                setUser({
                                    ...
                                    displayName: formatAddress(metamaskAddress),
                ...
                const productSync = supabase.channel('products_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'products' }, () => fetchProducts()).subscribe();
                return () => { productSync.unsubscribe(); if (authListener?.subscription) authListener.subscription.unsubscribe(); };
    ...
    const fetchProducts = async () => {
        if (!isSupabaseConfigured) { setProducts(INITIAL_PRODUCTS); return INITIAL_PRODUCTS; }
        try {
            ...
            if (data) {
                const mapped = data.map(item => {
                    ...
                    return {
                        ...
                        images: item.images || [], description: item.description, isFeatured: item.is_featured,
    ...
    const mapAndSetOrders = (data: any[]) => {
        ...
        const mapped = data.map((o: any) => ({
            ...
            shippingAddress: o.shipping_address || o.shipping_info || o.shippingAddress || null,
            notes: o.notes || '',
    ...
    const addProduct = async (p: Product) => {
        ...
        try {
            const { error } = await supabase.from('products').insert([{
                id: p.id, name: p.name, price: p.price, category: p.category, images: p.images,
                description: p.description, is_featured: p.isFeatured, sizes: p.sizes,
                size_inventory: p.sizeInventory, nft_metadata: p.nft
            }]);
            if (error) throw error;
            await autoCommit({ message: generateProductAddedMessage(p.name) });
        } catch (err) { setProducts(prev => prev.filter(prod => prod.id !== p.id)); addToast('Failed to add product.', 'error'); }
    };
    ...
    const updateProduct = async (updated: Product) => {
        ...
        try {
            const { error } = await supabase.from('products').update({
                name: updated.name, price: updated.price, category: updated.category, images: updated.images,
                description: updated.description, is_featured: updated.isFeatured, sizes: updated.sizes,
                size_inventory: updated.sizeInventory, nft_metadata: updated.nft, archived: updated.archived
            }).eq('id', updated.id);
            if (error) throw error;
            await autoCommit({ message: generateProductUpdatedMessage(updated.name) });
        } catch (err) { if (original) setProducts(prev => prev.map(p => p.id === updated.id ? original : p)); addToast('Update failed.', 'error'); }
    };
        ...
        if (provider === AuthProvider.METAMASK) {
            try {
                ...
                if (data) {
                    ...
                    setUser({
                        uid: 'user_eth_' + data.address, displayName: formatAddress(data.address), email: null,
    ...
    const pickGiveawayWinner = async (id: string, count: number) => {
        setGiveaways(prev => prev.map(g => {
            if (g.id === id) {
                ...
                const winners = [];
                for (let i = 0; i < count && entries.length > 0; i++) winners.push(entries.splice(Math.floor(Math.random() * entries.length), 1)[0]);
    ...
    const connectMetaMaskWallet = async (address?: string) => {
        ...
        try {
            let addr = address;
            if (!addr) { const data = await connectWallet(); if (!data) return; addr = data.address; }
