 ...
 * A signature is replayable inside the window (a stateless serverless function
 * cannot hold nonces), so the window is kept to ten minutes: well past any
 * real MetaMask sign-in, short enough that a captured login is stale almost
 * immediately. The far side is bounded too, so a message stamped far in the
 * future can never mint an ever-valid login.
 */
...
/**
 * The message a wallet signs to log in. The address is lowercased so the
 * signed bytes are deterministic — signature recovery matches exact bytes.
 */
export function buildAdminWalletLoginMessage(address: string, issuedAt: number): string {
    return `SGCoalition admin login\nAddress: ${String(address).toLowerCase()}\nIssued: ${issuedAt}`;
}

/**
 * Strict parser — anything not matching the contract byte-for-byte is
 * rejected, so a crafted message cannot smuggle fields past the server.
 */
export function parseAdminWalletLoginMessage(message: string): { address: string; issuedAt: number } | null {
    const match = LOGIN_MESSAGE_RE.exec(String(message || ''));
    if (!match) return null;
    return { address: match[1].toLowerCase(), issuedAt: Number(match[2]) };
}
