...
2. **Vercel REST API.** Requires a REST API bearer token. `VERCEL_TOKEN` (the standard personal access token from Vercel Dashboard → Settings → Tokens) is **NOT** present. The only available token env var is `VERCEL_OIDC_TOKEN`, which is the OIDC flow Vercel uses for CI↔Vercel trust during deploy actions — its scope is typically limited to the deploy trigger, not the broader `/v13/deployments/{id}/build-output` lookup endpoint. **This REST path was NOT actively tested in this diagnostic** (would require extracting the token + manually running `curl -H "Authorization: Bearer $VERCEL_OIDC_TOKEN" https://api.vercel.com/...`); it remains a not-fully-exhausted capture path that a future operator with shell access may attempt. The conclusion "Auth fails" above is therefore conditional on the assumption that OIDC tokens cannot authorize against arbitrary REST endpoints — not directly verified.
