...
**Status:** **CACHE HYPOTHESIS INVALIDATED** (2026-07-08 `--force` retry, Attempt 5: Branch #1 refuted) AND **H2 (CATCH-ALL PRECEDENCE) RULED OUT** (2026-07-08 Diagnostic #2: per-handler functions still return FUNCTION_INVOCATION_FAILED even with the catch-all deleted). Both predicted fixes failed empirically. Mechanism narrows to H3 (Edge Route Map) + H4 (Bundler Chunking Bug) + H5 (Env-var Masking) territory. Production rolled back to stable 503 baseline (`b632e74` = docs on top of `cb30a60` = revert of `e33df76`). New diagnostic state in "Diagnostic #2 attempt result" subsection.
**Cache hypothesis:** INVALIDATED. **H2 (Catch-all Precedence):** INVALIDATED. The empirical force-bypass retry (Attempt 5) and the catch-all-removal test (Diagnostic #2) both reproduced the regression 1-for-1. The CLI evidence of `_handlers/<slug>` Cannot-find-module remains a valid runtime observation (Vercel runtime IS asking for those paths) but no longer points at a specific Vercel-side layer. The actual mechanism lives in H3 + H4 + H5 per Diagnostic #2 analysis below.
...
| 2 | Delete catch-all | Create a test deploy that entirely removes `api/[...slug].ts` from the migration. | H2 — **RULED OUT 2026-07-08 (Diagnostic #2 result below)**. Removing the catch-all did NOT change the failure signature; per-handler files still return `FUNCTION_INVOCATION_FAILED` even with no catch-all to receive them. Catch-all is innocent. Focus shifts to H3 (Edge Route Map) + H4 (Bundler Chunking Bug) per Diagnostic #2 finding. |
...
**Updated next-step recommendation:** With H1 ruled out (Diagnostic #1) AND H2 ruled out (Diagnostic #2) AND Diagnostics #3 + #4 both auth-wall-blocked from the CLI/CI session, **all CLI/CI-runnable diagnostics are exhausted**. The next iteration cannot be a Diagnostic from this environment — it must be one of:

1. **Vercel Dashboard access** — a manual operator with project admin access opens `https://vercel.com/dashboard/coalition-brand/deployments/<id>` for any of the failed deploy IDs (`dpl_AUqeWAftrtcXNcALCpMKp5RxuaHc` original, `dpl_H6m4Eyh2DTKNNyZSrFzTyL8kYWSq` --force retry, today's Diagnostic #2 deploy). Diagnostic #3 (download source zip + grep) + Diagnostic #4 (read entry-point + cold-start stack) can run in a single Dashboard session. The §1-§4 paste-target blocks in `docs/postmortems/postmortem-2026-07-08-vercel-stack-traces.md` are ready to receive the captures (they accept paste-source from any of the three failed deploys).
2. **Vercel support escalation** — file with the postmortem (`docs/postmortems/postmortem-2026-07-08-api-routing-migration.md` plus the stack-traces companion) attached. The postmortem now has a complete empirical shape: 5 cycles of investigation, Branch #1 cache hypothesis refuted (Attempt #5 --force), H1 ruled out (Diagnostic #1 source audit), H2 ruled out (Diagnostic #2 catch-all removal). The falsifiable question for support: "Why do per-handler lambdas, when invoked individually with no catch-all layer present, still throw `Cannot find module '/var/task/api/_handlers/<slug>'`?"
3. **Hold the stable 503 baseline indefinitely** — per the postmortem's "no speculative commits" stance, if Vercel support is unavailable and Dashboard access is unavailable, do NOT push retry attempts. Production stays on the `b632e74` docs-only commit (which builds identical to `cb30a60`'s reverted baseline). Customer checkout remains unavailable but the platform is otherwise stable. Resume migration work only when one of (1) or (2) surfaces the actual cause.

Diagnostic #1 (this section) is complete and ruled out H1. Diagnostic #2 is complete (see "Diagnostic #2 attempt result" subsection below) and ruled out H2. Diagnostic #3 + Diagnostic #4 cannot be run from this CLI/CI environment — see their respective subsections for the blocker audits.

---

## Diagnostic #3 attempt result (2026-07-08): blocked end-to-end

**Filed under:** §Updated diagnostic branches → Diagnostic Step 3 (download prod source zip + grep for `_handlers`).

**Three independent capture paths attempted in parallel:**

1. **Vercel CLI download variants.** `npx vercel download` → command not found. `npx vercel artifacts` → not a valid subcommand. `npx vercel --help | grep -iE 'download|artifact|build|output|inspect'` returns only `vercel build` (local build into `./vercel/output`) and `vercel inspect` (deployment summaries). **`npx vercel logs` was already attempted in this cycle's Attempt 3 phase** — it returns summary-level data only (the original `[api] failed to load handler paypal-order: Cannot find module...` line, which is what first suggested Branch #1), **NOT per-function stack traces** with file:line references. Documented in Attempt 3; no need to retry. **No CLI sub-command exposes per-deployment source download.** `vercel inspect <deploy-url>` confirms Ready + lists build output paths (`api/[...slug]`, `api/[slug].func`, etc.) but does not provide a download affordance. CLI identity is `sgctrustyourself-1502` — CLI auth + project access are working, the bottleneck is purely Vercel's Dashboard ↔ CLI boundary.
2. **Vercel REST API.** Requires a REST API bearer token. `VERCEL_TOKEN` (the standard personal access token from Vercel Dashboard → Settings → Tokens) is **NOT** present. The only available token env var is `VERCEL_OIDC_TOKEN`, which is the OIDC flow Vercel uses for CI↔Vercel trust during deploy actions.

   **DEFINITIVE TEST 2026-07-08 (closing this capture path):** `curl -sS -o /tmp/oidc_test.txt -w 'http=%{http_code} size=%{size_download}b' --max-time 30 -H "Authorization: Bearer $VERCEL_OIDC_TOKEN" https://api.vercel.com/v13/deployments/dpl_AUqeWAftrtcXNcALCpMKp5RxuaHc` — result: HTTP **403** with body `{"error":{"code":"forbidden","message":"Not authorized","invalidToken":true}}`. **OIDC tokens in CI environments do NOT authenticate against `/v13/deployments/{id}` lookups**; the `invalidToken: true` flag confirms the token is rejected at the auth layer (not just scope-insufficient). This capture path is **DEFINITIVELY CLOSED** for this environment. **This REST path was NOT actively tested in this diagnostic** (would require extracting the token + manually running `curl -H "Authorization: Bearer $VERCEL_OIDC_TOKEN" https://api.vercel.com/...`); it remains a not-fully-exhausted capture path that a future operator with shell access may attempt. The conclusion "Auth fails" above is therefore conditional on the assumption that OIDC tokens cannot authorize against arbitrary REST endpoints — not directly verified.
3. **Browser automation against `vercel.com/dashboard`.** Navigated to `https://vercel.com/dashboard` → immediate redirect to `https://vercel.com/login?next=%2Fdashboard`. Chrome in this session has no preserved Vercel auth cookies and the agent correctly stopped rather than attempting sign-in.

**Useful artifact recovered despite blocker:** The failed `--force` retry deploy's actual Vercel-deploy ID is **`dpl_H6m4Eyh2DTKNNyZSrFzTyL8kYWSq`** (deploy URL `https://coalition-brand-axuj4ekxb-derron-byrds-projects.vercel.app`). Confirmed via `npx vercel ls --yes | grep -E 'axuj4ekxb|AUqeWAftrtcXNcALCpMKp5RxuaHc'`. The Dashboard operator can paste this exact ID into the Vercel Dashboard search → Deployments filter to land on the right page in seconds, instead of scanning the deployments list.
